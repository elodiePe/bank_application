...
import { useFamilyChores, useUpdateChore } from '../hooks/useChores.js';
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div className="flex items-center justify-between gap-3 border-b border-slate-200 p-4 dark:border-slate-700">
        <p className="text-sm text-slate-500 dark:text-slate-400">
          L'enfant marque une corvée comme faite, un parent valide pour verser la récompense.
        </p>
        <button
          type="button"
            ...
            {chores.data.map((chore) => (
              ...
              >
                ...
                <button
                  ...
                  className={
                    chore.active
                      ? 'shrink-0 rounded-full border border-slate-300 px-3 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-60 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700'
                      : 'shrink-0 rounded-full bg-brand-100 px-3 py-1 text-xs font-medium text-brand-700 hover:bg-brand-200 disabled:opacity-60 dark:bg-brand-900/40 dark:text-brand-400 dark:hover:bg-brand-900/70'
                  }
                >
